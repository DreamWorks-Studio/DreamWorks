...
/*export const createPayment = async (req, res) => {
    try {
        ...
        // ✅ Validate required fields
        if (!bookingId || !amountPaid || !paymentMethod || !userId) {
            return res.status(400).json({ message: 'Missing required fields' });
        }
        ...
        if (booking.user._id.toString() !== userId.toString()) {
            return res.status(403).json({ message: 'Payment does not belong to the current user' });
        }
        ...
        if (existingPayment) {
            // User is making an additional payment towards a previous partial payment

            if (existingPayment.paymentStatus === 'completed') {
                return res.status(400).json({ message: 'Full payment already completed for this booking' });
            }

            // ✅ Ensure user does not overpay
            if (existingPayment.remainingAmount < amountPaid) {
        ...
        // ✅ Handle Card Payments
            ...
            if (!cardNumber || !expiryDate) {
                return res.status(400).json({ message: 'Card number and expiry date are required for card payments' });
            }
        ...
        const newPayment = new Payment({
            ...
            remainingAmount,
            cardNumber, // ✅ Always required but only saved if partial payment
            expiryDate, // ✅ Always required but only saved if partial payment
...
/*export const createPayment = async (req, res) => {
    try {
        ...
        if (!userId || !bookingId || !amountPaid || !paymentMethod) {
            return res.status(400).json({ message: 'Missing required fields' });
        }
        ...
        let paymentStatus = paymentMethod === 'cash' ? 'pending' : (paymentType === 'partial' ? 'partial' : 'completed');
        ...
        const maskedCardNumber = cardNumber ? '*'.repeat(cardNumber.replace(/\s/g, '').length - 4) + cardNumber.replace(/\s/g, '').slice(-4) : null;
        ...
        if (existingPayment) {
            ...
            existingPayment.paymentStatus = existingPayment.remainingAmount === 0 ? 'completed' : 'partial';
...
export const generateInvoice = async (req, res) => {
    try {
        ...
        if (!booking.packageId || !booking.packageId.price) {
            ...
            // Use packageType or another available field from booking
            doc.text(`${booking.packageType || 'Photography Service'}`, 60, rowY + 10);
            doc.text(`Date: ${serviceDate}`, 60, rowY + 30);
            doc.text(`Rs.${recentPayment.packagePrice.toFixed(2)}`, 450, rowY + 20);

            const totalY = rowY + 50;
            doc.rect(50, totalY, 500, 25).stroke();
            
            const totalAmount = recentPayment.totalAmount.toFixed(2);
        ...
        const serviceDate = new Date(booking.date).toLocaleDateString();
        doc.text(`${booking.packageId.packageType || 'Photography Service'}`, 60, rowY + 10);
        doc.text(`Date: ${serviceDate}`, 60, rowY + 30);
