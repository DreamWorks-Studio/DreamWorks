            ...
            if (paymentMethod === "cash") {
                ...
                if (selectMethodResponse.status === 201) {
                    ...
                    if (!invoiceResponse.ok) {
                        setPopupMessage(`Invoice generation failed: ${invoiceResponse.status}`);
                        setPopupType('error');
                    ...
                    setPopupMessage("Payment method selected successfully!");
                    setPopupType('success');
                    setShowSuccessPopup(true);

                    // Wait for success popup to close before showing invoice popup
                    setTimeout(() => {
                        setShowPopup(true); // Show your existing invoice popup
                    }, 1600); // Wait 1.6 seconds before showing the invoice popup
                } else {
                    const errorData = await selectMethodResponse.json();
                    setPopupMessage(`Failed to process cash payment: ${errorData.message || "Please try again."}`);
                    setPopupType('error');
                    setShowSuccessPopup(true);
                }
                            ...
                            <section className="relative">
                                ...
                                <div className="bg-white rounded-2xl border border-gray-200 overflow-hidden shadow-sm">
                                    <div className="p-6 space-y-6">
                                        {/* Card Payment Option */}
                                        <div
                                            className={`p-5 rounded-2xl border-2 cursor-pointer transition ${paymentMethod === 'card' ? 'border-amber-500 bg-amber-50' : 'border-gray-200 hover:border-gray-300'
                                                }`}
                                            onClick={() => setPaymentMethod('card')}
                                        >
                                            <div className="flex items-center">
                                                <div className={`h-5 w-5 rounded-full border-2 mr-3 flex items-center justify-center ${paymentMethod === 'card' ? 'border-amber-500' : 'border-gray-400'
                                                    }`}>
                                                    {paymentMethod === 'card' && (
                                        ...
                                        {/* Cash Payment Option */}
                                        <div
                                            className={`p-5 rounded-2xl border-2 cursor-pointer transition ${paymentMethod === 'cash' ? 'border-amber-500 bg-amber-50' : 'border-gray-200 hover:border-gray-300'
                                                }`}
                                            onClick={() => setPaymentMethod('cash')}
                                        >
                                            <div className="flex items-center">
                                                <div className={`h-5 w-5 rounded-full border-2 mr-3 flex items-center justify-center ${paymentMethod === 'cash' ? 'border-amber-500' : 'border-gray-400'
                                                    }`}>
                                                    {paymentMethod === 'cash' && (
                                                        <div className="h-2 w-2 rounded-full bg-amber-500"></div>
                                                    )}
                    ...
                    <div className="bg-white p-8 rounded-2xl shadow-lg max-w-md border border-gray-200">
                        ...
                        <div className="flex flex-col space-y-4">
                            <a
                                ...
                                className="inline-flex items-center justify-center bg-amber-500 hover:bg-amber-600 text-white py-3 px-4 rounded-full transition-colors focus:outline-none"
                                onClick={async (e) => {
                                    e.preventDefault();
                                    try {
                                        ...
                                        setPopupMessage('Invoice downloaded successfully!');
                                        setPopupType('success');
                                        setShowSuccessPopup(true);
                                    } catch (error) {
                                        console.error('Download error:', error);
                                        setPopupMessage('Failed to download invoice. Please try again.');
                                        setPopupType('error');
                                        setShowSuccessPopup(true);
                                    }
                                }}
